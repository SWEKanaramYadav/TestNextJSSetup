...
router.isPhoneNumberOrEmail = function (inputString) {
    // Define a regular expression pattern for matching US phone numbers
    const phonePattern = /^\d{10}$/;

    // Define a regular expression pattern for matching email addresses
    const emailPattern = /^[\w\.-]+@[\w\.-]+\.\w+$/;

    // Check if the input string matches either the phone number or email pattern
    if (phonePattern.test(inputString)) {
        return "Phone Number";
    } else if (emailPattern.test(inputString)) {
